...
## Quais fórmulas os campos representam?

Os campos `a` e `b` configuram somente a parcela **fora de `T(...)`**, isto é, `a*n+b`. Por exemplo, escolher divisão por `3`, `a=2` e `b=7` representa `T(n)=T(n/3)+2*n+7`, com divisão inteira.

O programa aceita um único termo recursivo com coeficiente 1. Portanto, `T(n)=2*T(n/2)+8*T(n/4)+2` não pode ser representada pelas entradas atuais. Aceitá-la exigiria ampliar o código e a entrada para informar vários termos, cada um com seu coeficiente e sua redução.
